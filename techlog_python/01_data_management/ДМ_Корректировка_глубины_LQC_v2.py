...
for well in db.selectedWellList():
	gbd = 'GDB_LOGS'
	lqc = 'LQC'
	depthGbd = db.variableLoad(well, gbd, db.referenceName(well, gbd))[-1]
	depthLqc = db.variableLoad(well, lqc, db.referenceName(well, lqc))[-1]
	if depthGbd > depthLqc:
		td_new = round(depthLqc,0)
		print(well, round(depthGbd,1), td_new)
