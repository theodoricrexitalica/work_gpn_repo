    ...
    for line in file.readlines()[12:]:
    	...
    	if not "_K" in line:
    		if not "_BOT" in line:
		    	...
		    	data_txt.append(line.split(" ")[6] + ";" + line.split(" ")[4] + ";" + line.split(" ")[2] + ";" + line.split(" ")[1])
#print(data_txt)
db.currentChange("import")
...
for well in db.wellList():
	...
	for i in range(len(wells_txt)):
		if well == wells_txt[i]:
			try:
				...
				list_x.append(float(data_txt[i].split(";")[3]))
				list_y.append(float(data_txt[i].split(";")[2]))
			except:
				list_md.append(0)
				list_zones.append(" ")
				list_x.append(" ")
	...
	db.variableSave(well, "ZONES" + output_fld, "X", "X Coordinate", "m", list_x, 0, "double")
	db.variableSave(well, "ZONES" + output_fld, "Y", "Y Coordinate", "m", list_y, 0, "double")
