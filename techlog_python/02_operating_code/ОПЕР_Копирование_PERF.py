...
for well in db.selectedWellList():
	ds = "PERF"
	if db.datasetExists(well, ds):																													#Проверка наличия датасета с перфорацией и 
		print(well)																																			#снятие глубин перфораций
		md = db.variableLoad(well, ds, db.referenceName(well, ds))
		top_perf = []
		bot_perf = []
		for i in range(len(md))[1::2]:
			bot_perf.append(md[i])
		for i in range(len(md))[0::2]:
			top_perf.append(md[i])
		if 'G' not in well and db.datasetExists(well, "RIGIS_bdntc"):																#Проверка что скважина не ГС и наличие датасета RIGIS_bdntc
			...
			for i in range(len(perf)):
				perf[i] = -9999
			db.variableSave(well,  "RIGIS_bdntc", "PERF", "Perforation", "unitless", perf, 0, "double")			#Сохранение пустой переменной под перфорацию
			ind1 = []
			ind2 = []
			for i in range(len(md_bdntc)):																										#Поиск индексов в РИГИС-ГеоБД
			...
			for k in range(len(ind1)):
				print(md_bdntc[ind1[k]], '-',md_bdntc[ind2[k]] )
				for q in range(len(md_bdntc))[ind1[k]:ind2[k]]:
					perf[q] = 1																															#Заполнение пространства между индексами глубин перфораций
			...
			print(well, 'is HRZ or RIGIS_bdntc is absent')
			pass
	else:
		print(well, ds, 'is absent')
		pass
