...
ds_datascience_name = 'DS_Ety_18_6m'																			#Название датасета для результатов
percent = 90
...
for well in db.selectedWellList():
	if "G" in well:
		ds = "Index"
		var = "TVDBML"
		zone_name = db.variableLoad(well, ds_zone, 'ZONES')[-1]
		zone_indice = db.datasetZoneIndice(well, ds, ds_zone, zone_name, "Zone Name", 0)
		tvdbml = db.variableLoad(well, ds, var)
		ind1 = zone_indice[0]
		ind2 = zone_indice[1]
		hrz = tvdbml[ind1:ind2]																							#Указание диапазона согласно зонейшену для поиска моды по АО
		cc += 1
		...
		round_hrz = list(map(func, hrz))																				#Округление значений  глубины АО до 1 знака после запятой
