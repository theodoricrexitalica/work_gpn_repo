...
#DeclarationsEnd
ds_zone = 'ZONES_gm'																							#Указываем датасет с зонами
var_zone = 'БС9-2-3_TOP_S'																						#Указываем целевой пласт
...
for well_all in db.wellList():																								#Расчет расстояния между базовой и всеми скважинами в проекте
	if db.datasetExists(well_all, ds_zone):
...
for i in range(len(list_keys))[0:4]:
	print(dict_distance[list_keys[i]], list_keys[i])
